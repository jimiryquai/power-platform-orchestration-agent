            ...
            const response = {
              ...
              application: {
                ...
                clientSecret: result.data.clientSecret ? {
                  secretText: result.data.clientSecret.secretValue,
                  secretId: result.data.clientSecret.secretId,
                  displayName: result.data.clientSecret.displayName,
                  expiresAt: result.data.clientSecret.expiresAt
                } : undefined
              },
              credentials: result.data.clientSecret ? {
                clientId: result.data.appId,
                clientSecret: result.data.clientSecret.secretValue,
                tenantId: process.env.AZURE_TENANT_ID
              } : undefined,
              nextSteps: [
                "Save the client ID and secret for Azure DevOps service connections",
                "Use these credentials to create a Service-to-Service (S2S) user in Power Platform",
                "The app has been granted admin consent for Dynamics 365 and Power Platform permissions",
                "Store the credentials securely - the secret cannot be retrieved again"
              ],
              message: `Azure AD application '${applicationName}' created successfully with client secret`
            };
...
function generateAccessTokenFromCredentials(): string {
  if (process.env.AZURE_CLIENT_ID && process.env.AZURE_CLIENT_SECRET && process.env.AZURE_TENANT_ID) {
    ...
    // TODO: Implement actual service principal token generation
    return 'sp-generated-token';
  }
  
  if (process.env.AZURE_USE_INTERACTIVE_AUTH === 'true') {
    console.error('🔐 Using Azure CLI authentication for Microsoft Graph');
    // Get token from Azure CLI
    try {
      const { execSync } = require('child_process');
      const token = execSync('az account get-access-token --resource https://graph.microsoft.com --query accessToken -o tsv', { encoding: 'utf8' }).trim();
      console.error('✅ Successfully obtained Azure CLI token');
      return token;
    } catch (error) {
      console.error('❌ Failed to get Azure CLI token:', error instanceof Error ? error.message : 'Unknown error');
